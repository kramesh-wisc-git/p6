...
int main(int argc, char *argv[]) {
    ...
    while((opt = getopt(argc, argv, "r:d:i:b:")) != -1) {
        if(opt == 'r') { // raid mode
            ...
            if(rem) numb_inodes += (32 - rem); // align numb_inodes to a multiple of 32
        }
        else if(opt == 'b') { // number of data blocks
            numb_data_blocks = atoi(optarg);
            if(numb_data_blocks <= 0) exit(1);
            int rem = numb_data_blocks & (32-1);
            if(rem) numb_data_blocks += (32 - rem); // align numb_data_blocks to multiple of 32
        }
    }
    ...
    sb.d_bitmap_ptr = sb.i_bitmap_ptr + numb_inodes / 8;
    sb.i_blocks_ptr = sb.d_bitmap_ptr;
    int rem = sb.d_bitmap_ptr & (512-1);
